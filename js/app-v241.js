let D=load(),state={type:null,level:null,mode:null,qs:[],i:0,hits:0,correct:null,locked:false,shopCat:'casco',grid:null,path:[],target:[],daily:false};
...
const levelRewards={
 2:{type:'diamantes',amount:20,label:'20 diamantes'},
 3:{type:'item',id:'ce_comun_casco',label:'Casco espacial común'},
 4:{type:'item',id:'ce_comun_espada',label:'Espada espacial común'},
 5:{type:'item',id:'ce_comun_pecho',label:'Peto espacial común'},
 6:{type:'item',id:'ce_comun_escudo',label:'Escudo estelar común'},
 7:{type:'diamantes',amount:40,label:'40 diamantes'},
 8:{type:'item',id:'ce_comun_botas',label:'Botas espaciales comunes'},
 9:{type:'diamantes',amount:60,label:'60 diamantes'},
 10:{type:'item',id:'ce_comun_arma_energia',label:'Arma de energía común'}
};
const ACHIEVEMENTS=[
...
<div class="home-head">
  <div class="brand"><div class="brand-mark">AJ</div><div><h1>Aprendo jugando</h1><div class="muted">V2.4.1 · TEST equipamiento</div></div></div>
  ${diamond()}
</div>
${xpPanel()}
...
function itemPreview(i){
 return `<div class="item-preview real-asset-preview">
   <img src="${i.image}" alt="${i.name}" loading="lazy">
   <span class="quality-badge quality-${i.quality||'comun'}">${i.quality==='comun'?'COMÚN':(i.quality||'')}</span>
 </div>`
}

const SHOP_CATS=[
 ['casco','Casco'],['pecho','Pecho'],['guantes','Guantes'],['cinturon','Cinturón'],
 ['pierneras','Pierneras'],['botas','Botas'],['capa','Capa'],['arma','Armas'],['escudo','Escudo']
];

function shop(cat=state.shopCat){
 if(!SHOP_CATS.some(x=>x[0]===cat))cat='casco';
 state.shopCat=cat;
 layout(`
 <div class="top shop-head"><button class="btn secondary back" onclick="home()">← Volver</button>
   <div><h2>Tienda</h2><div class="muted">Caballero Espacial · Colección Común</div></div>${diamond()}
 </div>
 <div class="shop-layout">
   <div class="shop-avatar-panel">
     <div class="avatar-stage shop-avatar">${avatarHTML(D)}</div>
     <div class="shop-avatar-caption"><b>${D.perfil.nombre}</b><span>${Object.values(D.equipado).filter(Boolean).length} piezas equipadas</span></div>
     <button class="small reset" onclick="clearEquip()">Quitar todo</button>
   </div>
   <div class="shop-browser">
     <div class="tabs equipment-tabs">${SHOP_CATS.map(([id,label])=>`<button class="btn secondary tab ${id===cat?'active':''}" onclick="shop('${id}')">${label}</button>`).join('')}</div>
     <div class="shop">${GAME.items.filter(i=>i.cat===cat).map(i=>{
       const own=D.inventario.includes(i.id),eq=D.equipado[i.cat]===i.id;
       return `<div class="shop-card ${own?'owned':''} ${eq?'equipped':''}">
         ${itemPreview(i)}
         <div class="shop-info"><b>${i.name}</b>
           <div class="shop-price">${own?(eq?'✓ Equipado':'En tu colección'):i.price+' 💎'}</div>
           <div class="item-tag">Caballero Espacial · Común</div>
         </div>
         <button class="small buy" onclick="buy('${i.id}')">${own?(eq?'Quitar':'Equipar'):'Comprar'}</button>
       </div>`
     }).join('')||'<div class="empty-shop">Todavía no hay objetos en esta categoría.</div>'}</div>
   </div>
 </div>`)
}
...
function buy(id){
 ...
 if(!i)return;
 const own=D.inventario.includes(id),eq=D.equipado[i.cat]===id;
 if(!own){
  if(D.diamantes<i.price)return alert('No tienes suficientes diamantes');
  D.diamantes-=i.price;
  D.inventario.push(id);
  D.equipado[i.cat]=id;
 }else{
  D.equipado[i.cat]=eq?null:id;
 }
 checkAchievements();save(D);shop(i.cat)
}

function clearEquip(){
 D.equipado=emptyEquip();
 save(D);
 shop()
}
